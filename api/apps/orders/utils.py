...
from ..translators.models import Translator, TranslatorTraffic
...
# ============================================================= #
#  ВБУДОВАНИЙ АНАЛІЗАТОР ФАЙЛІВ (Word, PDF, Excel тощо)
# ============================================================= #
...
def _count_chars_from_parts(text_parts: list[str]) -> tuple[int, int]:
    # З'єднуємо частини тексту через \n (абзац), щоб Word-логіка не рахувала переходи між абзацами як пробіли
    full_text = "\n".join(p for p in text_parts if p)
    full_text = normalize_text(full_text, collapse_spaces=False)

    # Символи без пробілів: видаляємо ВСІ невидимі символи (\n, \r, \t, пробіл)
    chars_no_spaces = len(re.sub(r'\s', '', full_text))

    # Пробіли: рахуємо ТІЛЬКИ фізичні пробіли ' ' (Microsoft Word не вважає \n чи \r пробілом)
    space_count = full_text.count(' ')

    chars_with_spaces = chars_no_spaces + space_count
    return chars_with_spaces, chars_no_spaces
...
def analyze_file_content(file) -> dict:
    ...
    try:
        # -------------------------------------------------------- #
        # MICROSOFT OFFICE (WORD, POWERPOINT, EXCEL) via COM
        # -------------------------------------------------------- #
        if file_name.endswith(OFFICE_EXTENSIONS):
            if file_name.endswith(
                    ('.docx', '.dotx', '.dotm', '.docm', '.pptx', '.ppsx', '.potx', '.xlsx', '.xlsm', '.xltx',
                     '.xltm')):
                file.seek(0)
                try:
                    if zipfile.is_zipfile(file):
                        ...
                        with zipfile.ZipFile(file) as zf:
                            stats["images"] = sum(1 for n in zf.namelist() if '/media/' in n)
                except Exception:
                    pass
            ...
            try:
                if ext in ['.doc', '.docx', '.dotx', '.dotm', '.docm']:
                    app_instance = win32com.client.DispatchEx("Word.Application")
                    app_instance.Visible = False
                    app_instance.DisplayAlerts = False
                    doc = app_instance.Documents.Open(temp_path, ReadOnly=True, ConfirmConversions=False)
                    stats["pages"] = doc.ComputeStatistics(2, True)
                    stats["words"] = doc.ComputeStatistics(0, True)
                    stats["chars_no_spaces"] = doc.ComputeStatistics(3, True)
                    stats["chars_with_spaces"] = doc.ComputeStatistics(5, True)

                elif ext in ['.ppt', '.pptx', '.ppsx', '.potx']:
                    app_instance = win32com.client.DispatchEx("PowerPoint.Application")
                    doc = app_instance.Presentations.Open(temp_path, ReadOnly=True, WithWindow=False)
                    stats["pages"] = doc.Slides.Count
                    for slide in doc.Slides:
                        for shape in slide.Shapes:
                            if shape.HasTextFrame and shape.TextFrame.HasText:
                                text = shape.TextFrame.TextRange.Text.replace('\x0b', '\n').replace('\r', '\n')
                                stats["words"] += len(text.split())
                                stats["chars_with_spaces"] += len(text.replace('\n', ' '))
                                stats["chars_no_spaces"] += len(text.replace('\n', '').replace(' ', ''))
                            if shape.HasTable:
                                for r in range(1, shape.Table.Rows.Count + 1):
                                    for c in range(1, shape.Table.Columns.Count + 1):
                                        c_text = shape.Table.Cell(r, c).Shape.TextFrame.TextRange.Text.replace('\x0b',
                                                                                                               '\n').replace(
                                            '\r', '\n')
                                        stats["words"] += len(c_text.split())
                                        stats["chars_with_spaces"] += len(c_text.replace('\n', ' '))
                                        stats["chars_no_spaces"] += len(c_text.replace('\n', '').replace(' ', ''))

                elif ext in ['.xls', '.xlsx', '.xlsm', '.xltx', '.xltm']:
                    app_instance = win32com.client.DispatchEx("Excel.Application")
                    app_instance.Visible = False
                    app_instance.DisplayAlerts = False
                    doc = app_instance.Workbooks.Open(temp_path, ReadOnly=True)
                    stats["pages"] = doc.Sheets.Count
                    for sheet in doc.Sheets:
                        used_range = sheet.UsedRange
                        if used_range.Count == 1 and not used_range.Value: continue
                        for cell in used_range:
                            if cell.Value is not None:
                                val = str(cell.Value)
                                stats["words"] += len(val.split())
                                stats["chars_with_spaces"] += len(val)
                                stats["chars_no_spaces"] += len(val.replace(' ', ''))

                stats_done = True

            except Exception as e:
                logger.error(f"Помилка MS Office COM: {e}")

            finally:
                if doc:
                    try:
                        doc.Close(SaveChanges=False) if hasattr(doc, 'Close') else None
                    except:
                        pass
                if app_instance:
                    try:
        ...
        # -------------------------------------------------------- #
            ...
            try:
                ...
                pdf_text_parts = []
                for page in doc:
                    # get_text("blocks") дозволяє розбивати PDF логічно по абзацах
                    blocks = page.get_text("blocks")
                    for b in blocks:
                        if b[6] == 0:  # 0 означає що це текстовий блок
                            block_text = b[4].strip()
                            if block_text:
                                # В межах одного абзацу PDF всі \n — це штучні переноси рядків, перетворюємо на пробіл
                                block_text = re.sub(r'\n+', ' ', block_text)
                                block_text = re.sub(r' {2,}', ' ', block_text)
                                pdf_text_parts.append(block_text.strip())

                    stats["images"] += len(page.get_images(full=True))
                doc.close()

                if pdf_text_parts:
                    # Склеюємо абзаци через \n.
                    # Наш _count_chars_from_parts НЕ рахуватиме \n як пробіл (як у Word)
                    text_parts.append("\n".join(pdf_text_parts))
            except Exception as e:
                logger.error(f"Помилка читання PDF через PyMuPDF: {e}")
        ...
        if not stats_done:
            if text_parts:
                stats["chars_with_spaces"], stats["chars_no_spaces"] = _count_chars_from_parts(text_parts)
            if stats["pages"] == 0:
