...
from .models import OrderEditorReview, TranslationQuality, OrderStatusHistory
from .serializers import OrderCreateSerializer, OrderTrafficSerializer
from .serializers import RejectTranslationSerializer, \
    ApproveTranslationSerializer, ChangeStatusSerializer
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def change_status(self, request, pk=None):
        ...
        new_status_id = serializer.validated_data['status_id']

        status_obj = get_object_or_404(Status, pk=new_status_id)

        order.status = status_obj
        order.client_status = status_obj
        order.translator_status = status_obj
        ...
        OrderStatusHistory.objects.create(
            ...
            translator_status=status_obj
        )

        return Response({
            "message": f"Статус змінено на {status_obj.name}",
            "status": {
                "id": status_obj.id,
