...
import { EditorStatsTable } from "./EditorStatsTable"
...
export function EditorStatsPage() {
    const [range, setRange]           = useState<DateRange | undefined>(undefined)
    const [open, setOpen]             = useState(false)
    const [searchQuery, setSearchQuery] = useState("")
    const typingTimer                 = useRef<NodeJS.Timeout | null>(null)

    const { data, loading, fetchStats } = useEditorStats()
    ...
    // ─── KPI карточки ─────────────────────────────────────────────────────────
    const totalEditors   = data?.length || 0
    const totalOrders    = data?.reduce((acc, e) => acc + (e.total_orders || 0), 0) || 0
    const totalPages     = data?.reduce((acc, e) => acc + Number(e.total_pages || 0), 0) || 0
    const totalRevenue   = data?.reduce((acc, e) => acc + Number(e.total_revenue || 0), 0) || 0
    const avgMargin      = data?.length
        ? data.reduce((acc, e) => acc + Number(e.avg_margin_percent || 0), 0) / data.length
        : 0
            ...
            <div className="flex flex-col h-full min-h-screen bg-[#F8FAFC] p-4 sm:p-8">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-slate-500 text-sm mt-1">
                        Кількість замовлень, перевірені сторінки та виручка по кожному редактору
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
                    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm flex flex-col">
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <span className="text-sm font-semibold text-slate-700">Редакторів</span>
                        </div>
                        <p className="text-2xl font-bold text-slate-900">{totalEditors}</p>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm flex flex-col">
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <span className="text-sm font-semibold text-slate-700">Перевірено замовлень</span>
                        </div>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm flex flex-col">
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <span className="text-sm font-semibold text-slate-700">Всього сторінок</span>
                        </div>
                ...
                <div className="bg-white p-4 rounded-t-2xl border border-slate-100 border-b-0 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="relative w-full sm:w-[320px]">
                        ...
                        <Input
                            placeholder="Пошук редактора за іменем..."
                ...
                <div className="bg-white rounded-b-2xl border border-slate-100 shadow-sm overflow-hidden flex-1">
                    <EditorStatsTable data={data} loading={loading} />
                </div>
            </div>
        </>
    )
