...
import { useManagerDetail, useManagerOrders } from "../hooks/useManagerStats"
            ...
            <Table>
                <TableHeader className="bg-white">
                    <TableRow className="hover:bg-transparent border-b border-slate-100">
                        <TableHead className="h-12 pl-6 font-semibold text-slate-700 w-[180px]">
                            ID / Дата
                        </TableHead>
                ...
                <TableBody>
                    {loading ? (
                        ...
                        <TableRow>
                            <TableCell colSpan={5} className="h-32 text-center text-slate-400">
                                Замовлення відсутні за вибраними параметрами
                            </TableCell>
                        </TableRow>
                    ) : (
                        data.map((order) => {
                            ...
                            return (
                                <TableRow
                                    key={order.id}
                                    className="transition-colors hover:bg-slate-50 border-b border-slate-50 last:border-0"
                                >
                                    {/* 1. ID + дата створення */}
...
export default function ManagerDetailPage() {
    ...
    const { data: orders, loading: ordersLoading, fetchOrders } = useManagerOrders(managerId)

    // Ефект завантаження даних при зміні дати
    useEffect(() => {
        ...
        fetchDetail(p)
        // При завантаженні також враховуємо поточний текст пошуку, якщо він є
                ...
                <div className="bg-white border border-slate-100 rounded-2xl shadow-sm mb-6 overflow-hidden">
                    <div className="p-6 flex flex-col md:flex-row md:items-start justify-between gap-6">
                        <div className="flex items-start gap-4">
                            <div className="w-16 h-16 rounded-full bg-violet-100 border border-violet-200 flex items-center justify-center text-2xl font-bold text-violet-600 shrink-0 shadow-sm">
                                {manager?.full_name?.[0]?.toUpperCase() ?? "M"}
                            </div>
                            <div className="mt-1">
                                <h1 className="text-2xl font-bold text-slate-900 tracking-tight">
                                    {manager?.full_name ?? "Завантаження..."}
                                </h1>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                            <Popover open={open} onOpenChange={setOpen}>
                                <PopoverTrigger asChild>
                                    <Button
                                        variant="outline"
                                        className="w-[260px] justify-start gap-2 bg-white font-normal border-slate-200 rounded-lg shadow-sm text-sm text-slate-500"
                                    >
                    ...
                    <div className="bg-slate-50/50 border-t border-slate-100 p-6 flex flex-wrap items-start gap-8 md:gap-16">
                        ...
                        <div>
                            ...
                            {(summary?.overdue_orders_count || 0) > 0 ? (
                                <div className="inline-flex items-center gap-1.5 bg-red-50 text-red-600 border border-red-100 rounded-full px-3 py-1 text-sm font-semibold">
                                    <AlertCircle className="w-3.5 h-3.5" />
                                    {summary?.overdue_orders_count}
                                </div>
                            ) : (
                    ...
                    <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
                        ...
                        <div className="-mx-2">
                            {loading
                                ...
                                : <ReactApexChart options={makeAreaOptions(ordersCategories, "#8b5cf6")} series={ordersSeries} type="area" height={220} />
                            }
                        </div>
                    </div>
                    <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
                        <h3 className="text-base font-bold text-slate-900 mb-1">Дохід (Виручка)</h3>
                        <p className="text-2xl font-bold text-slate-900 mb-2">{formatCurrency(summary?.total_revenue || 0)}</p>
                        <div className="-mx-2">
                            {loading
                                ? <ChartSkeleton height={200} />
                                : <ReactApexChart options={makeBarOptions(revenueCategories, "#6366f1")} series={revenueSeries} type="bar" height={220} />
                            }
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden mb-10">
                    <div className="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="text-xs text-slate-400 mt-0.5">
                                Замовлення де менеджер приймав або здавав роботу
                            </p>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                            <div className="relative w-full sm:w-[300px]">
                                ...
                                <Input
                                    ...
                                    className="pl-9 bg-slate-50 border-slate-200 focus-visible:ring-violet-500 h-9"
                                    value={searchQuery}
                                    onChange={handleSearchChange}
                                />
                            </div>

                            <div className="inline-flex items-center gap-1.5 bg-violet-50 text-violet-700 border border-violet-100 rounded-full px-3 py-1.5 text-xs font-semibold shrink-0">
