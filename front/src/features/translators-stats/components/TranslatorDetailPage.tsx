...
import {
    ChevronLeft, CalendarIcon, ChevronDown,
    TrendingUp, TrendingDown, AlertCircle,
    ShoppingBag, Star, X, Wallet, Percent, Search, Info,
} from "lucide-react"
import { cn } from "@/src/lib/utils"

import { useTranslatorDetail, useTranslatorOrders } from "../hooks/useTranslatorStats"
import type { TranslatorOrder } from "../types"
...
const STATUS_NAMES: Record<number, string> = {
    1: "New", 2: "Done", 3: "Rejected", 4: "In Progress",
    5: "Paid", 7: "In Review", 8: "In Checking", 9: "Checked", 11: "Revision",
}
...
function OrdersTable({ data, loading }: { data: TranslatorOrder[]; loading: boolean }) {
    return (
            ...
            <Table>
                ...
                <TableBody>
                    {loading ? (
                        ...
                        <TableRow><TableCell colSpan={5} className="h-32 text-center text-slate-400">Замовлення відсутні за вибраний період</TableCell></TableRow>
                    ) : (
                        data.map((order) => {
                                ...
                                <TableRow key={order.id} className="transition-colors hover:bg-slate-50 border-b border-slate-50 last:border-0">
                                    ...
                                    <TableCell className="py-4">
                                        <div className="flex items-center gap-1.5">
                                            ...
                                            <span className={overdue ? "text-sm font-medium text-red-600" : "text-sm text-slate-600"}>
                                                {formatDate(order.deadline)}
                                            </span>
                                        </div>
                                    </TableCell>
                                    <TableCell className="py-4">
                                        <span className="text-sm text-slate-600">{order.page_count ?? "—"}</span>
                                    </TableCell>
                                    <TableCell className="py-4 max-w-[260px]">
                                        {order.client_comment
                                            ? <span className="text-sm text-slate-600 truncate block">{order.client_comment}</span>
...
export default function TranslatorDetailPage() {
    ...
    const translatorId = Number(params?.id)

    const [range, setRange] = useState<DateRange | undefined>(undefined)
    const [open, setOpen]   = useState(false)

    // Стейт та реф для пошуку
    const [searchQuery, setSearchQuery] = useState("")
    const typingTimer = useRef<NodeJS.Timeout | null>(null)

    const { data, loading,                    fetchDetail } = useTranslatorDetail(translatorId)
    const { data: orders, loading: ordersLoading, fetchOrders } = useTranslatorOrders(translatorId)

    useEffect(() => {
        const p = range?.from && range?.to
            ? { start_date: formatYMD(range.from), end_date: formatYMD(range.to) }
            : undefined
        fetchDetail(p)
        // При завантаженні враховуємо поточний текст пошуку
    ...
    const translator = data?.translator_info
    const summary    = data?.summary
    const margin     = Number(summary?.avg_margin_percent || 0)
    const marginGood = margin >= 50

    // ── Графіки ───────────────────────────────────────────────────────────────
    const ordersCategories = data?.orders_chart?.map((p) =>
        p.date ? new Date(p.date).toLocaleString("uk-UA", { day: "numeric", month: "short" }) : ""
    ) ?? []
    const ordersSeries = [{ name: "Замовлення", data: data?.orders_chart?.map((p) => p.count || 0) ?? [] }]

    const revenueCategories = data?.revenue_chart?.map((p) =>
        p.date ? new Date(p.date).toLocaleString("uk-UA", { day: "numeric", month: "short" }) : ""
    ) ?? []
    const revenueSeries = [{ name: "Виручка", data: data?.revenue_chart?.map((p) => Number(p.amount || 0)) ?? [] }]
    ...
    const makeBarOptions = (categories: string[], color: string): ApexCharts.ApexOptions => ({
        ...
        fill: { type: "gradient", gradient: { shade: "light", type: "vertical", gradientToColors: ["#10b981"], stops: [0, 100] } },
        grid: { borderColor: "#f1f5f9", xaxis: { lines: { show: false } }, yaxis: { lines: { show: true } } },
        xaxis: { type: "category", categories, labels: { style: { colors: "#94a3b8", fontSize: "11px" } }, axisBorder: { show: false }, axisTicks: { show: false } },
        yaxis: { tickAmount: 4, labels: { style: { colors: "#94a3b8", fontSize: "11px" }, formatter: (v) => `${Number(v || 0).toLocaleString("uk-UA")} ₴` } },
        tooltip: { theme: "light", y: { formatter: (v) => `${Number(v || 0).toLocaleString("uk-UA")} ₴` } },
    })
                ...
                <div className="bg-white border border-slate-100 rounded-2xl shadow-sm mb-6 overflow-hidden">
                    ...
                    <div className="p-6 flex flex-col md:flex-row md:items-start justify-between gap-6">
                        <div className="flex items-start gap-4">
                            <div className="w-16 h-16 rounded-full bg-emerald-100 border border-emerald-200 flex items-center justify-center text-2xl font-bold text-emerald-600 shrink-0 shadow-sm">
                                {translator?.full_name?.[0]?.toUpperCase() ?? "T"}
                            </div>
                            <div className="mt-1">
                                <h1 className="text-2xl font-bold text-slate-900 tracking-tight">
                                    {translator?.full_name ?? "Завантаження..."}
                                </h1>
                                <p className="text-slate-500 mt-0.5 text-sm">{translator?.email}</p>
                                {translator?.rating !== undefined && (
                                    <div className="mt-2 inline-flex items-center gap-1.5 text-[11px] font-medium text-amber-600 bg-amber-50 px-2.5 py-1 rounded-full border border-amber-100">
                                        <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
                                        Рейтинг: {Number(translator.rating).toFixed(1)}
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                            <Popover open={open} onOpenChange={setOpen}>
                                <PopoverTrigger asChild>
                                    <Button variant="outline" className={cn("w-[260px] justify-start gap-2 bg-white font-normal border-slate-200 rounded-lg shadow-sm text-sm", !range && "text-slate-500")}>
                                ...
                                <PopoverContent className="w-auto p-0" align="end">
                                    <Calendar mode="range" selected={range}
                                              onSelect={(val) => { setRange(val); if (val?.from && val?.to) setOpen(false) }}
                    ...
                    <div className="bg-slate-50/50 border-t border-slate-100 p-6 flex flex-wrap items-start gap-8 md:gap-16">
                        <div>
                            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Замовлень</p>
                        ...
                        <div>
                            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Середній чек</p>
                            <p className="text-xl font-bold text-slate-900">{formatCurrency(summary?.avg_order_value || 0)}</p>
                        </div>
                        <div>
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                <p className={`text-xl font-bold ${marginGood ? "text-emerald-600" : "text-amber-600"}`}>
                                    {margin.toFixed(1)}%
                                </p>
                            </div>
                        </div>
                        <div>
                            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Ревізії</p>
                            {(summary?.revision_count || 0) > 0 ? (
                                <div className="inline-flex items-center gap-1.5 bg-amber-50 text-amber-600 border border-amber-100 rounded-full px-3 py-1 text-sm font-semibold">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                    <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
                        <h3 className="text-base font-bold text-slate-900 mb-1 flex items-center gap-2">
                            Кількість виконаних замовлень <Info className="w-4 h-4 text-slate-400" />
                        </h3>
                        <p className="text-2xl font-bold text-slate-900 mb-2">{summary?.total_orders || 0}</p>
                        <div className="-mx-2">
                            {loading
                                ? <ChartSkeleton height={200} />
                                : <ReactApexChart options={makeAreaOptions(ordersCategories, "#10b981")} series={ordersSeries} type="area" height={220} />
                            }
                        </div>
                    </div>
                    <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
                        <h3 className="text-base font-bold text-slate-900 mb-1 flex items-center gap-2">
                            Витрачені кошти (Виручка) <Info className="w-4 h-4 text-slate-400" />
                        </h3>
                        <p className="text-2xl font-bold text-slate-900 mb-2">{formatCurrency(summary?.total_revenue || 0)}</p>
                        <div className="-mx-2">
                            {loading
                                ? <ChartSkeleton height={200} />
                                : <ReactApexChart options={makeBarOptions(revenueCategories, "#059669")} series={revenueSeries} type="bar" height={220} />
                            }
                        </div>
                        <div className="flex items-center gap-3 mt-3 pt-3 border-t border-slate-50">
                            <div className="flex items-center gap-2 bg-slate-50 text-slate-700 rounded-lg px-3 py-1.5 text-xs font-medium">
                                ...
                                Середній чек <span className="font-bold ml-1">{formatCurrency(summary?.avg_order_value || 0)}</span>
                            </div>
                            <div className="flex items-center gap-2 bg-slate-50 text-slate-700 rounded-lg px-3 py-1.5 text-xs font-medium">
                                <Percent className="w-3.5 h-3.5 text-emerald-500" />
                                Маржа <span className={`font-bold ml-1 ${marginGood ? "text-emerald-600" : "text-amber-600"}`}>{margin.toFixed(1)}%</span>
                            </div>
                        </div>
                    </div>
                </div>

                {/* ─── Таблиця замовлень з Пошуком ──────────────────────────── */}
                <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden mb-10">
                    <div className="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="text-xs text-slate-400 mt-0.5">Замовлення цього перекладача</p>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                            <div className="relative w-full sm:w-[300px]">
                                ...
                                <Input
                                    ...
                                    className="pl-9 bg-slate-50 border-slate-200 focus-visible:ring-emerald-500 h-9"
                                    value={searchQuery}
                                    onChange={handleSearchChange}
                                />
                            </div>

                            <div className="inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-full px-3 py-1.5 text-xs font-semibold shrink-0">
